#!/usr/bin/env python3
"""Lightweight structural validator for the mojo-expert skill."""
...
def main() -> None:
    ...
    trigger_evals = [entry for entry in evals if entry.get("category") == "trigger"]
    labels = {entry.get("trigger_expectation") for entry in trigger_evals}
    if labels and labels != {"should_trigger", "should_not_trigger"}:
        fail(
            "trigger evals must include both should_trigger and should_not_trigger "
            "cases when any trigger evals are present"
        )
    ...
    print("PASS: mojo-expert skill structure is valid")
